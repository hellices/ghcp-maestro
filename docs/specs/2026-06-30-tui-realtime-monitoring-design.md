...
- Per-phase **token totals** alongside agent counts and elapsed time (parity with
  a dynamic-workflows progress view — see §4b).
...
### Stretch goals (v2 — full dynamic-workflows parity)
...
## 4b. Parity target — dynamic multi-agent workflows

ghcp-maestro is a GitHub Copilot CLI port of the orchestrator-workers pattern that
leading agent CLIs ship as **dynamic workflows** (plan → fan-out subagents in parallel →
cross-check → single synthesized answer; 16 concurrent / 1000 per run; saved as
reusable scripts). The two line up almost 1:1 on the runtime, but the
`/workflows` **progress view** is the one capability ghcp-maestro lacks, and it is
exactly what this feature delivers. Such progress views typically offer:
...
2. Token total: show input+output combined, or break out input/output/cache?
   ...
   single "token total" column such progress views show.)
