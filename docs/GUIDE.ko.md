...
호스트가 run 시작 **전에** 각 `@경로`(현재 디렉터리 기준)를 읽어 파일명과
...
- run 당 최대 4개 파일; 파일당 16,000자 (초과분은 명시적 마커와 함께 잘림),
  합계 48,000자.
- 파일이 없거나 읽을 수 없으면 즉시 중단 — 토큰 소비 전.
- `/maestro brainstorm` 도 같은 방식으로 `@file` 참조 지원.
- run 매니페스트는 원본 라인을 보존하므로 `/maestro-resume` 은 같은 파일을
  다시 읽는다 (스펙 파일이 사라졌으면 깔끔하게 실패).
- 모든 하위 작업 프롬프트가 스펙을 포함하므로 큰 스펙 × 넓은 fan-out 은
  토큰 비용을 배가시킨다. 게이트의 규모 추정에 프롬프트 크기가 반영된다.
