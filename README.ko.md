...
줄어든다. 각 에이전트에는 넉넉한 타임아웃이 있다 (리서치 에이전트는 기본 10 분);
아주 긴 리서치 실행은 `GHCP_MAESTRO_TIMEOUT_MS=<ms>` 로 늘릴 수 있다. 일시적
에이전트 실패(API 오류, rate limit)는 지수 백오프로 자동 재시도한다 — 기본 1회,
`GHCP_MAESTRO_RETRIES=<n>` 로 조절 (`0` 이면 비활성).

**비용 가시성과 토큰 예산.**
fan-out 전 게이트에 실행 규모 추정이 표시되고, 하위 작업이
`GHCP_MAESTRO_LARGE_RUN_AGENTS` 개 이상(기본 5)이면 경고가 나온다.
`GHCP_MAESTRO_BUDGET_TOKENS=<n>` (`500k` / `2m` 축약 가능) 으로 run 상한을
걸 수 있다: 상한 도달 시 진행 중 에이전트는 마무리하고, 아직 시작 안 한
에이전트는 건너뛴 뒤 run 을 soft-stop 한다 — 나중에 `/maestro-resume` 으로
이어서 실행. `/maestros` 는 에이전트별 · 전체 토큰 사용량을 실시간으로 보여준다.
